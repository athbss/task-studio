...
Taskmaster Circle - A web-based interface for the Taskmaster CLI task management system. This UI template provides a visual, interactive way to view and manage tasks from `.taskmaster` JSON files with real-time updates.
...
- **Focus**: Working implementation over perfect code
- **Integration First**: Get .taskmaster file reading working before UI polish
- **Local Development**: Optimize for local developer experience like Prisma Studio
- **Incremental Features**: Start read-only, add write operations via CLI later
